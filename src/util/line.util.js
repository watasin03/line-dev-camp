...
exports.getProfile = async (userId) => {
  try {
    ...
    if (profile == undefined) {
      const url = `${process?.env?.NEXT_PUBLIC_LINE_MESSAGING_API}/profile/${userId}`;

      const response = await axios.get(url, {
        headers: {
          Authorization: `Bearer ${process.env.NEXT_PUBLIC_LINE_MESSAGING_ACCESS_TOKEN}`,
...
exports.isAnimationLoading = async (userId) => {
  try {
    const url = `${process.env.NEXT_PUBLIC_LINE_MESSAGING_API}/chat/loading/start`;
...
exports.replyWithLongLived = async (token, payload) => {
  try {
    const url = `${process.env.NEXT_PUBLIC_LINE_MESSAGING_API}/message/reply`;
...
async function issueStatelessAccessToken() {
  try {
    const response = await axios.post(
      process.env.NEXT_PUBLIC_LINE_MESSAGING_OAUTH_ISSUE_TOKENV3,
      {
        grant_type: "client_credentials",
        client_id: process.env.NEXT_PUBLIC_LINE_MESSAGING_CHANNEL_ID,
        client_secret: process.env.NEXT_PUBLIC_LINE_MESSAGING_CHANNEL_SECRET,
      },
      {
        headers: {
...
exports.verifySignature = (originalSignature, body) => {
  const signature = crypto
    .createHmac("SHA256", process.env.NEXT_PUBLIC_LINE_MESSAGING_CHANNEL_SECRET)
...
exports.getContent = async (message, messageId) => {
  const url = `${process.env.NEXT_PUBLIC_LINE_DATA_MESSAGING_API}/message/${messageId}/content`;
  const response = await axios.get(url, {
    headers: {
      Authorization: `Bearer ${process.env.NEXT_PUBLIC_LINE_MESSAGING_ACCESS_TOKEN}`,
    },
